...
@section('title','Accounts | Manage Currency')
...
<div class="card">
    ...
    <div class="row">
        <div class="col-md-2 col-md-offset-10">
            <button class="btn btn-primary  p-10" data-toggle="modal" data-target="#categoryAdd">Add Currency</button>
        </div>
    </div>
    ...
    <table id="data-table-command" class="table table-striped table-vmiddle">
        ...
        <tbody>
            <?php $i = 0; ?>
            @foreach( $currencys as $category )
            <?php $i ++; ?>
            <tr>
                <td>{{ $i }}</td>
                <td>{{ $category->currency_name }}</td>
                <td>{{ $category->currency_code }}</td>
                <td>{{ $category->currency_symbol }}</td>
...
<div class="modal fade" id="categoryAdd" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form role="form" method="POST" action="{{ route('currency.store') }}" class="remove-margin-p">
                {{ csrf_field() }}
                        ...
                        <div class="panel-body">
                            <div class="form-group ">
                                <div class="fg-line">
                                    ...
                                    <input type="text" class="form-control input-sm" name="name" placeholder="currency name" value="" />
                                </div>
                                    @if ($errors->has('name'))
                            ...
                            <div class="form-group ">
                                <div class="fg-line">
                                    ...
                                    <input type="text" class="form-control input-sm" name="code" placeholder="currency code" value="" />
                                </div>
                                    @if ($errors->has('code'))
                                        <p class="text-right text-danger">{{$errors->first('code')}}</p>
                                    @endif
                            </div>
                            <div class="form-group ">
                                <div class="fg-line">
                                    <label for="symbol">Currency Symbol</label>
                                    <input type="text" class="form-control input-sm" name="symbol" placeholder="currency symbol" value="" />
                                </div>
                                    @if ($errors->has('symbol'))
                                        <p class="text-right text-danger">{{$errors->first('symbol')}}</p>
                                    @endif
                            </div>
                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group ">
                                <div class="fg-line">
                                    <label for="title">Currency Name</label>
                                    <input type="text" class="form-control input-sm" id="name" name="name" placeholder="currency name" value="" />
                                </div>
                                    @if ($errors->has('name'))
                                        <p class="text-right text-danger">{{$errors->first('name')}}</p>
                                    @endif
                            ...
                            <div class="form-group ">
                                <div class="fg-line">
                                    <label for="title">Currency Code</label>
                                    <input type="text" class="form-control input-sm" id="code" name="code" placeholder="currency code" value="" />
                                </div>
                                    @if ($errors->has('code'))
                                        <p class="text-right text-danger">{{$errors->first('code')}}</p>
                                    @endif
                            ...
                            <div class="form-group ">
                                <div class="fg-line">
                                    <label for="symbol">Currency Symbol</label>
                                    <input type="text" class="form-control input-sm" id="symbol" name="symbol" placeholder="currency symbol" value="" />
                                </div>
                                    @if ($errors->has('symbol'))
                                        <p class="text-right text-danger">{{$errors->first('symbol')}}</p>
                                    @endif
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                $( ".delete_category" ).click(function() {
                    ...
                    var url=location.origin+'/currency/'+account_id;
                ...
                $( ".edit_category" ).click(function() {
                    ...
                    var update_url=location.origin+'/currency/'+account_id;

                    var url=location.origin+'/currency/'+account_id+'/edit';

                    $('.edit_category_form').attr('action',update_url);
                            
                        $.get(url,function(data){
                            $('#name').val(data['currency_name']);
                            $('#code').val(data['currency_code']);
                            $('#symbol').val(data['currency_symbol']);
